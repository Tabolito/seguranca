...
def increment_key(key):
    ...
    new_key = bytes(key_list)

    return new_key
def bruteforce(ciphertext):
  key = b'\00' * 16  # 16 bytes of zero
  try:
...
def decrypts(key, ciphertext):
  resposta = b""
  for i in range(len(ciphertext)//16+1):
    if(i * 16 == len(ciphertext)):
      break
  #  if (i+1)*16 > len(ciphertext):
  #    resposta += decrypt(key, pad_to_16_bytes(ciphertext[i*16:]))
  #  else:
    resposta += decrypt(key, ciphertext[i*16:(i+1)*16])

  return resposta
def encrypts(key, plaintext):
  resposta = b""
  for i in range(len(plaintext)//16+1):
    if(i * 16 == len(plaintext)):
      break
    if (i+1)*16 > len(plaintext):
      resposta += encrypt(key, pad_to_16_bytes(plaintext[i*16:]))
    else:
      resposta += encrypt(key, plaintext[i*16:(i+1)*16])
  return resposta
